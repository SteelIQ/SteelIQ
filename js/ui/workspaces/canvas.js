...
(function (global) {
   ...
   */
/**
 * 1. FIX: Inward $135° Seismic Hooks (Anchored Inside Core Concrete around Corner Rebar)
 */
function addStirrupHooks(group, vertices, tieDia, angleDeg, centroid) {
  if (!vertices || vertices.length < 3) return;
  
  // Anchorage Corner: Top-Left vertex inside cover
  const p0 = vertices[0];
  const px = p0.x * MM_TO_PX;
  const py = p0.y * MM_TO_PX;

  const strokeW = Math.max(1.8, tieDia * MM_TO_PX);
  const hookLen = Math.max(14, tieDia * 7 * MM_TO_PX);

  // Hook 1: $135° Inward angle towards concrete core (Bottom-Right direction)
  const h1x = px + hookLen * Math.cos(Math.PI / 4);
  const h1y = py + hookLen * Math.sin(Math.PI / 4);

  group.appendChild(svg('line', {
    x1: px, y1: py, x2: h1x, y2: h1y,
    stroke: 'var(--text-secondary)', 'stroke-width': strokeW, 'stroke-linecap': 'round'
  }));

  // Hook 2: Parallel Overlapping Tail (offset by bar thickness)
  const off = strokeW + 1;
  const h2x = (px + off) + hookLen * Math.cos(Math.PI / 4);
  const h2y = (py + off) + hookLen * Math.sin(Math.PI / 4);

  group.appendChild(svg('line', {
    x1: px + off, y1: py + off, x2: h2x, y2: h2y,
    stroke: 'var(--text-secondary)', 'stroke-width': strokeW, 'stroke-linecap': 'round'
  }));
}

/**
 * 2. FIX: Architectural Dimensions with Padded Badges Clear of Lines
 */
function addDim(group, x, y, length, text, vertical) {
  ...
  const fontWidth = 7;
  const pad = 12;
  const bgWidth = text.length * fontWidth + pad;
  ...
  if (vertical) {
    ...
    // Vertical Text Badge (Right-aligned clear of line)
    const bg = svg('rect', { x: x + 10, y: y + length / 2 - 10, width: bgWidth, height: 20, rx: 3, fill: 'var(--bg-panel)', stroke: 'var(--annotate)', 'stroke-width': 1 });
    const t = svg('text', { x: x + 10 + bgWidth / 2, y: y + length / 2 + 4, fill: 'var(--annotate)', 'font-size': 10.5, 'font-weight': 700, 'text-anchor': 'middle', 'font-family': 'var(--font-mono)' });
    t.textContent = text;
    ...
    // Horizontal Text Badge (Centering background box over line)
    const bg = svg('rect', { x: x + length / 2 - bgWidth / 2, y: y - 10, width: bgWidth, height: 20, rx: 3, fill: 'var(--bg-panel)', stroke: 'var(--annotate)', 'stroke-width': 1 });
    const t = svg('text', { x: x + length / 2, y: y + 4, fill: 'var(--annotate)', 'font-size': 10.5, 'font-weight': 700, 'text-anchor': 'middle', 'font-family': 'var(--font-mono)' });
    t.textContent = text;
    g.appendChild(bg); g.appendChild(t);
  }
  group.appendChild(g);
...
/**
 * 3. FIX: Dynamic-Width Tie Callout Box (Prevents Text Overflow)
 */
function addTieCallout(group, outline, cover, tie) {
  ...
  // Calculated width ensuring full text fits
  const bgWidth = calloutText.length * 6.5 + 16;
  const bg = svg('rect', { x: endX, y: endY - 11, width: bgWidth, height: 20, rx: 3, fill: 'var(--bg-panel)', stroke: 'var(--border-strong)', 'stroke-width': 1 });
  const t = svg('text', { x: endX + 8, y: endY + 3, fill: 'var(--text-secondary)', 'font-size': 9.5, 'font-weight': 600, 'font-family': 'var(--font-mono)' });
...
/**
 * 4. FIX: High-Contrast Concrete Hatch Pattern inside buildColumnSvg <defs>
 */
// Replace the hatch pattern definition inside buildColumnSvg with this higher opacity line:
const hatchPat = svg('pattern', {
  id: 'concrete-hatch',
  width: '12', height: '12',
  patternUnits: 'userSpaceOnUse',
...
hatchPat.appendChild(svg('line', {
  ...
  stroke: 'var(--text-muted)', 'stroke-width': '1.2', 'stroke-opacity': '0.5'
}));
